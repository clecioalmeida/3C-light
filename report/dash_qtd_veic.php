...
$query_transp = "SELECT date_format(t1.dt_conhecimento, '%d/%m/%Y') as data_vg, count(DISTINCT t1.id_veiculo) as total_vg, round(sum(t1.nr_peso_kg)/1000,2) as nr_peso, SUM(t1.km_adic) as km_cobrado
from tb_conhecimento t1
left join tb_veiculo t2 on t1.id_veiculo = t2.nr_veiculo
...
$query_mes = "SELECT month(t1.dt_conhecimento) as mes_vg, year(t1.dt_conhecimento) as ano_vg, count(DISTINCT nr_manifesto) as total_vg, round(sum(t1.nr_peso_kg)/1000,2) as nr_peso, SUM(t1.km_adic) as km_cobrado
from tb_conhecimento t1
left join tb_veiculo t2 on t1.id_veiculo = t2.nr_veiculo
where month(t1.dt_conhecimento) = '$date' and t1.nr_fatura > 0
...
?>
<?php include 'chart_qtd_veic.php';?>
<div class="row">
  <fieldset>
    <section>
      <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6" id="printGraph">
        <header>
          ...
          <h2>VEÍCULOS EXPEDIDOS POR DIA - MÊS ATUAL</h2>
        </header>
        <div>
    ...
    <section>
      <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6" id="printGraph">
        <header>
          <span class="widget-icon"></span>
          <h2>VEÍCULOS EXPEDIDOS POR MÊS</h2>
        </header>
        <div>
          <div class="widget-body no-padding">
        ...
        <table id="dt_basic" class="table table-bordered" width="100%">
          <thead style="color:black">
            <tr>
              ...
              <th style="width: 40px">TOTAL DE VIAGENS FATURADAS</th>
              <th style="width: 50px">PESO (t)</th>
              <th style="width: 50px">KM COBRADO</th>
            </tr>
          </thead>
          <tbody>
            <?php
            while ($cte = mysqli_fetch_assoc($res_transp)) {
              ...
              <tr>
                ...
                <td style="text-align: right;"><?php echo $cte['total_vg'];?></td>
                <td style="text-align: right;"><?php echo $cte['nr_peso'];?></td>
                <td style="text-align: right;"><?php echo $cte['km_cobrado'];?></td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
          ...
          <tbody>
            ...
            while ($cte_mes = mysqli_fetch_assoc($res_mes)) {
              ...
              <tr>
                ...
                <td style="text-align: right;"><?php echo $cte_mes['total_vg'];?></td>
                <td style="text-align: right;"><?php echo $cte_mes['nr_peso'];?></td>
                <td style="text-align: right;"><?php echo $cte_mes['km_cobrado'];?></td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
...
<script type="text/javascript">
  ...
  config = {
    ...
    xkey: 'data_man',
    ykeys: ['total_veic'],
    labels: ['Total de veículos expedidos'],
    pointSize: 5,
    fillOpacity: 0.6,
    behaveLikeLine: true,
    resize: true,
