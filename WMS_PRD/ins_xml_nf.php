		...
		if(move_uploaded_file($arquivo['tmp_name'][$controle], $destino)){
			...
			foreach ($Arrxml->NFe as $key => $xml) {
				...
				if(mysqli_num_rows($res_nf) > 0){
					...
					$sql = "insert into tb_nf_entrada (nr_fisc_ent, id_rem, id_dest, dt_emis_ent, chavenfe, nr_peso_ent, vl_tot_nf_ent, qtd_vol_ent, tp_vol_ent, fl_status, fl_empresa, cod_rec, cod_ag, usr_create, dt_create) values ('$nr_nf_formulario', '$cod_rem', '$cod_dst', '$dt_emissao', '$nfe_chave', '$nr_peso', '$vl_mercadoria', '$nr_volume', '$tp_vol', 'A', '$cod_cli', '$id_rec', '$id_rec', '$id', '$date')";
					...
					if (mysqli_affected_rows($link1) > 0) {
						...
						for ($i=0; $i <=1000 ; $i++) { 
							...
							if(!empty($xml->infNFe->det[$i]->prod->cProd)){
								...
								/*$sql_cod = "select cod_produto, cod_prod_cliente, nm_produto from tb_produto where nm_produto = '$nm_produto'";
								$res_cod = mysqli_query($link, $sql_cod);
								if(mysqli_num_rows($res_cod) > 0){
									$dados=mysqli_fetch_assoc($res_cod);
									$produto = $dados['cod_prod_cliente'];

								}*/

								if($unidade == "KG"){

									$sql_prd = "insert into tb_nf_entrada_item (cod_nf_entrada, produto, nm_produto, nr_peso_unit, vl_unit, ds_unid, nr_ean, fl_status, fl_imp) values ('$nova_nf', '$produto', '$nm_produto', '$nr_qtde', '$nr_valor', '$unidade', '$nr_ean', 'A', 'S')";
									$res_prd = mysqli_query($link, $sql_prd);

								}else{

									$sql_prd = "insert into tb_nf_entrada_item (cod_nf_entrada, produto, nm_produto, nr_qtde, vl_unit, ds_unid, nr_ean, fl_status, fl_imp) values ('$nova_nf', '$produto', '$nm_produto', '$nr_qtde', '$nr_valor', '$unidade', '$nr_ean', 'A', 'S')";
									$res_prd = mysqli_query($link, $sql_prd);
