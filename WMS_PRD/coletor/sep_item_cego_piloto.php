...
if($nr_pedido != "0"){
	...
	$sql_ped = "select t1.cod_col,t1.nr_pedido, t1.produto, t1.ds_galpao, t1.ds_prateleira, t1.ds_coluna, t1.ds_altura, t1.nr_qtde_col as nr_qtde_col, t1.nr_qtde_conf, t2.cod_produto, t2.nm_produto, t2.cod_prod_cliente, t3.nome, t1.cod_estoque, t4.fl_status, t5.nr_qtde, t6.ds_unid, t7.id, t1.cod_estoque
	...
	left join tb_pedido_conferencia t5 on t1.cod_col = t5.cod_col and coalesce(t5.fl_status,'A') <> 'E'
	left join tb_pedido_coleta_produto t6 on t1.nr_pedido = t6.nr_pedido and t1.produto = t6.produto
	left join tb_etiqueta t7 on t4.cod_estoque = t7.cod_estoque
	...
	where nr_pedido = '$nr_pedido' and coalesce(fl_status,'A') <> 'E'";
	...
	<div data-role="page" id="one" class="jqm-demos jqm-home" style="background-color: white">
		<div data-role="header" class="jqm-header" style="height: 50px">
			...
			<a href="picking_cego_piloto.php" class="jqm-search-link ui-btn ui-btn-icon-notext ui-corner-all ui-icon-arrow-l ui-nodisc-icon ui-alt-icon ui-btn-left"></a>
						...
						<div role="content">
							...
							<div class="widget-body">
								<form id="formPedido">
											...
											<tbody style="font-size: 16px">
												...
												while ($dados_ped = mysqli_fetch_assoc($res_ped)) {?>
													<tr class="tblRows" data_prd="<?php echo $dados_ped['cod_produto']; ?>" data_rua="<?php echo $dados_ped['ds_prateleira']; ?>" data_col="<?php echo $dados_ped['ds_coluna']; ?>" data_alt="<?php echo $dados_ped['ds_altura']; ?>" data_qtd="<?php echo $dados_ped['nr_qtde_col']; ?>"  data_glp="<?php echo $dados_ped['ds_galpao']; ?>">
														<td><?php echo $dados_ped['cod_prod_cliente']." - Cód.Etq.: ".$dados_ped['id']." - ".$dados_ped['nm_produto']; ?></td>
														<td><?php echo $dados_ped['ds_prateleira']."-".$dados_ped['ds_coluna']."-".$dados_ped['ds_altura']." - Cod.Estoque: ".$dados_ped['cod_estoque']; ?></td>
														...
														<td>
															<a href="conf_cego_piloto.php?cod_ped=<?php echo $nr_pedido;?>&cod_prd=<?php echo $dados_ped['cod_prod_cliente'];?>&rua=<?php echo $dados_ped['ds_prateleira'];?>&col=<?php echo $dados_ped['ds_coluna']; ?>&alt=<?php echo $dados_ped['ds_altura']; ?>&qtd=<?php echo $totalQtde; ?>&galpao=<?php echo $dados_ped['ds_galpao']; ?>&cod_estoque=<?php echo $dados_ped['cod_estoque']; ?>&cod_col=<?php echo $dados_ped['cod_col']; ?>&id_etq=<?php echo $dados_ped['id']; ?>&nr_qtde=<?php echo $dados_ped['nr_qtde_col']; ?>" class="btn btn-primary btn-lg btn-block" data-transition="pop" style="text-decoration: none">
									...
									<fieldset>
										<div class="form-group">
											<button type="submit" class="ui-btn ui-shadow-icon ui-btn-a" id="btnFinConfPedCegoPl" value="<?php echo $nr_pedido;?>" style="float: right;margin-left: 5px;background-color: #16a085;text-shadow: none;color:white;border-color: #fdfbfb">Finalizar</button>
										</div>
									</fieldset>
								</form>
							</div>
			...
			<div class="row">
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					<div class="jarviswidget jarviswidget-sortable" id="wid-id-2" data-widget-colorbutton="false" data-widget-editbutton="false" role="widget">
						...
						<div role="content">
							<h3>Não há dados para mostrar.</h3>
						</div>
					</div>
				</article>
			</div>
