...
nr_serial, nr_qtde, ds_lp, ds_ano, ds_kva, ds_fabr
...
if (mysqli_num_rows($res_conf) > 0) {
    ...
    while ($conf = mysqli_fetch_assoc($res_conf)) {
        ...
        $sql_ins = "INSERT into tb_posicao_pallet (
            ...
            fl_status, fl_bloq, fl_tipo, fl_empresa, usr_create, dt_create, ds_lp, ds_ano, ds_kva, ds_fabr
            ) values (
                '" . $cod_produto . "','" . $id_end . "','" . $ds_galpao . "','" . $ds_rua . "','" . $ds_coluna . "',
                '" . $ds_altura . "', '" . $nr_qtde . "','" . $nr_serial . "','" . $cod_recebimento . "','A','N','D',
                '" . $cod_cli . "', '" . $id . "','" . $date . "','" . $ds_lp . "','" . $ds_ano . "','" . $ds_kva . "','" . $ds_fabr . "'
                )";
        $res_ins = mysqli_query($link2, $sql_ins);

        if(mysqli_affected_rows($link2) > 0){

            $sql_prd = "UPDATE tb_recebimento_ag set 
            fl_status = 'F', usr_aloca = '".$id."', dt_aloca = '" . $date . "' 
            where cod_recebimento = '" . $cod_recebimento . "'";
