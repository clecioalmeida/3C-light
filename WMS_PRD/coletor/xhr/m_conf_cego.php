...
<script type="text/javascript">
    ...
    $(document).ready(function(){
    	$('#local').change(function(event){
    		...
    		var local = $('#local').val();
    		var galpao_conf = "<?php echo $galpao;?>";
	        var rua_conf = "<?php echo $rua;?>";
	        var col_conf = "<?php echo $col;?>";
	        var alt_conf = "<?php echo $alt;?>";
    		if($('#local').val() == ''){
	...
	$(document).ready(function(){
	    $(document).on('click','#btnSaveConfProd',function(event){
	        ...
	        if($('#barcode').val() == ''){
	            alert("Favor bipar o volume!");
	        }else if(nr_qtde_conf != qtd) {
	        	alert("A quantidade digitada deve ser a mesma do pedido!");
	        }else{

	        	if(prd == barcode){
		            $.ajax
		            ({
		                url:"xhr/conf_picking_cego.php",
		                method: "POST",
		                dataType:'json',
		                data:{prd:prd,rua:rua,col:col,alt:alt,qtd:qtd,galpao:galpao,barcode:barcode,pedido:pedido,nr_qtde_conf:nr_qtde_conf},
		                success:function(j)
		                {
					        	...
					        	for(var i=0;i < j.length;i++){
			                    	...
			                    	//var conferido = j[i].info;
									$('#TotalConferido').html(total_conf);
									//$('#total').html(conferido);
							}
		                }
		            });
	            	return false;
	    ...
	    $(document).on('click', '#closeModal',function() {
			window.location.href = "sep_item_cego.php?nr_pedido=<?php echo $nr_pedido;?>";
   		});
