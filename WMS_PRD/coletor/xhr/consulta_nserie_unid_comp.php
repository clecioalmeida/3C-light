...
if(isset($end[0]) && isset($end[1]) && isset($end[2]) && isset($end[3])){
	...
	if($dados['status_sap'] == 'DEPS' && $dados['status_usr'] == 'DEPS' && $dados['ds_centro'] == $ds_centro && $dados['ds_deposito'] == $ds_deposito){
		...
		$sql_comp = "select t1.id_produto, t1.n_serie
		from tb_nserie t1
		where t1.n_serie_comp = '$nr_serial' and id_produto = '$cod_prd' and t1.fl_status = 'A'";
		$res_comp = mysqli_query($link, $sql_comp);
		while ($dados = mysqli_fetch_assoc($res_comp)) {

			$upd_comp = "update tb_nserie set cod_pedido = '$nr_ped', ds_prateleira = '$rua', ds_coluna = '$col', ds_altura = '$alt', fl_status = 'C' where id_produto = '$id_produto' and n_serie = '".$dados['n_serie']."'";
			$res_upd_comp = mysqli_query($link1, $upd_comp);
			...
			if(mysqli_affected_rows($link1) > 0){
				...
				$retorno = array(
					'info'		=> "<p style='background-color: #98FB98'> Produto: ".$id_produto." N.série: ".$dados['n_serie']."</p>",
					'text' 		=> "<p style='background-color: #98FB98'>N.série:".$dados['n_serie']." inserido.</p>",
