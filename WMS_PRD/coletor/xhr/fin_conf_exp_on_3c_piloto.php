...
while ($parte = mysqli_fetch_assoc($res_col)) {
    ...
    if($cod_estoque == "0"){
        ...
        $ds_obs = "Produto não existe na alocação. Favor verificar o saldo ou o endereço apontado na separação.";
        ...
        $sql = "insert into tb_ocorrencias (nm_ocorrencia, tipo, ds_responsavel, nm_depto, criticidade, dt_abertura, fl_status, cod_origem, ds_obs, fl_empresa, user_create, dt_create) values ('$ds_obs', '', 'id', 'Conferencia', 'A', '$date', 'A', '$pedido', '', '$cod_cli', '$id', '$date')";
