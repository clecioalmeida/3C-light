...
$ds_lp = $_POST['barcodeConsLpPrd'];

$query_conf = "SELECT t1.produto, t1.ds_galpao, t1.ds_prateleira, t1.ds_coluna, t1.ds_altura, round(t1.nr_qtde, 0) as nr_qtde, t2.nome, t3.nm_produto, date_format(t1.dt_create,'%d/%m/%Y') as dt_create, COALESCE(t1.ds_lp, 0) as ds_lp, t3.unid
    from tb_posicao_pallet t1
    left join tb_armazem t2 on t1.ds_galpao = t2.id
    left join tb_produto t3 on t1.produto = t3.cod_prod_cliente
    where t1.ds_lp = '$ds_lp' and t1.nr_qtde > 0 and t1.fl_empresa = '$cod_cli' and t2.fl_situacao = 'A' and t1.fl_status = 'A'
...
if(mysqli_num_rows($res_conf) > 0){
    ...
    $retorno[] = array(
            'retorno' => 'Produto não encontrado com esse LP.',
    );
    echo(json_encode($retorno));
