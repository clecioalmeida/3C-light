...
$query_init="select sum(nr_qtde) as total_qtde, sum(nr_qtde_conf) as total_conf
...
if($total == $qtde){
	$upd_col="update tb_coleta_pedido set fl_status = 'P', usr_col = '$id', dt_col = now() where nr_pedido = '$pedido'";
	$res_upd_col=mysqli_query($link, $upd_col);

	$upd_prd="update tb_pedido_coleta_produto set fl_status = 'P', usr_fim_conf = '$id', dt_fim_conf = now() where nr_pedido = '$pedido'";
	$res_upd_prd=mysqli_query($link, $upd_prd);

	$upd_ped="update tb_pedido_coleta set fl_status = 'P' where nr_pedido = '$pedido'";
