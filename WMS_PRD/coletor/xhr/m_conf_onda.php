    ...
    $(document).ready(function(){
    	$('#local').change(function(event){
    		...
    		if($('#local').val() == ''){
	        	...
	        	$.ajax
	            ({
	                url:"xhr/conf_local_onda.php",
	                method: "POST",
	                dataType: 'json',
	                data:{onda:onda,local:local,end:end,prd:prd,galpao_conf:galpao_conf,rua_conf:rua_conf,col_conf:col_conf,alt_conf:alt_conf},
	                success:function(j){
	    ...
	    $(document).on('change','#barcode',function(event){
	        ...
	        if($('#barcode').val() == ''){
	        	...
	        	if(prd == barcode){
		            $.ajax
		            ({
		                url:"xhr/conf_picking_onda.php",
		                method: "POST",
		                dataType:'json',
		                data:{prd:prd,rua:rua,col:col,alt:alt,qtd:qtd,galpao:galpao,barcode:barcode,onda:onda},
		                success:function(j){
