...
$sql_ped = "SELECT t1.cod_ped, t1.nr_pedido, t1.produto, t2.cod_prod_cliente, t2.nm_produto, t1.nr_qtde, coalesce(t1.nr_qtde_exp,0) as nr_qtde_exp, 
coalesce(t1.ds_lp,0) as ds_lp, t1.ds_serial, t3.produto
from tb_pedido_coleta_produto t1
left join tb_coleta_pedido t3 on t1.cod_ped = t3.cod_ped
left join tb_produto t2 on t3.produto = t2.cod_prod_cliente
...
<div data-role="page" id="one" class="jqm-demos jqm-home" style="background-color: white">
	<div data-role="header" class="jqm-header" style="height: 50px">
		...
		<a href="expede_conv.php" class="jqm-search-link ui-btn ui-btn-icon-notext ui-corner-all ui-icon-arrow-l ui-nodisc-icon ui-alt-icon ui-btn-left"></a>
										...
										<tbody style="font-size: 10px">
											...
											while ($dados_ped = mysqli_fetch_assoc($res_ped)) {?>
												<tr class="tblRows" data_prd="<?php echo $dados_ped['produto']; ?>" data_qtd="<?php echo $dados_ped['nr_qtde']; ?>">
													...
													<td>
														<a href="conf_exp_con.php?cod_ped=<?php echo $dados_ped['cod_ped'];?>&nr_ped=<?php echo $nr_pedido;?>" class="btn btn-primary btn-lg btn-block" data-transition="pop" style="text-decoration: none">
