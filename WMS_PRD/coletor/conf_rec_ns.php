...
$query_conf="select count(id) as total from tb_nserie where cod_rec = '$cod_recebimento'";
	...
	<div role="main" class="ui-content" style="background-color: white">
		...
		<div class="row">
			<div class="conferido" id="conferido">
				<h4 id="TotalConferidoNs">Cadastrados:<?php echo number_format($conf,0,",","");?></h4>
				<h4 id="retConfNs" style="display: none"></h4>
			</div>
		</div>
		<div class="row">
			<form id="form_cad_ns" method="POST" action="">
				<div class="col-md-12">
					<div class="form-group">
						<input type="hidden" name="cod_rec" id="cod_rec" value="<?php echo $cod_recebimento; ?>">
						<input type="text" id="ins_qtde_ns" name="ins_qtde_ns" class="form-control" required="" style="text-align: right;"  autofocus="true"/>
					</div>
					<div class="form-group" id="retNSOr">
					</div>
				</div>
			</form>
		</div>
		<a href="rec_item_ns.php?cod_recebimento=<?php echo $cod_recebimento;?>" class="ui-btn ui-shadow ui-corner-all ui-btn-a">Fechar</a>
	</div>
</div>