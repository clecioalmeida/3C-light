...
$sql_ped = "select t1.cod_col,t1.nr_pedido, t1.produto, t1.ds_galpao, t1.ds_prateleira, t1.ds_coluna, t1.ds_altura, sum(t1.nr_qtde_col) as nr_qtde_col, t1.nr_qtde_conf, t2.cod_produto, t2.nm_produto, t2.cod_prod_cliente, t3.nome, t1.cod_estoque, t4.fl_status, t5.nr_qtde
...
<div data-role="page" id="one" class="jqm-demos jqm-home" style="background-color: white">
	<div data-role="header" class="jqm-header" style="height: 50px">
		...
		<a href="picking_serial_cego.php" class="jqm-search-link ui-btn ui-btn-icon-notext ui-corner-all ui-icon-arrow-l ui-nodisc-icon ui-alt-icon ui-btn-left"></a>
						...
						<form id="formPedido">
									...
									<tbody style="font-size: 11px">
										...
										while ($dados_ped = mysqli_fetch_assoc($res_ped)) {?>
											<tr class="tblRows" data_prd="<?php echo $dados_ped['cod_produto']; ?>" data_rua="<?php echo $dados_ped['ds_prateleira']; ?>" data_col="<?php echo $dados_ped['ds_coluna']; ?>" data_alt="<?php echo $dados_ped['ds_altura']; ?>" data_qtd="<?php echo $dados_ped['nr_qtde_col']; ?>"  data_glp="<?php echo $dados_ped['ds_galpao']; ?>">
												...
												<td>
													<a href="conf_cego_serial.php?cod_ped=<?php echo $nr_pedido;?>&cod_prd=<?php echo $dados_ped['cod_prod_cliente'];?>&rua=<?php echo $dados_ped['ds_prateleira'];?>&col=<?php echo $dados_ped['ds_coluna']; ?>&alt=<?php echo $dados_ped['ds_altura']; ?>&qtd=<?php echo $totalQtde; ?>&galpao=<?php echo $dados_ped['ds_galpao']; ?>&cod_estoque=<?php echo $dados_ped['cod_estoque']; ?>&cod_col=<?php echo $dados_ped['cod_col']; ?>" class="btn btn-primary btn-lg btn-block" data-transition="pop" style="text-decoration: none">
													...
													<a href="conf_cego_serial_range.php?cod_ped=<?php echo $nr_pedido;?>&cod_prd=<?php echo $dados_ped['cod_prod_cliente'];?>&rua=<?php echo $dados_ped['ds_prateleira'];?>&col=<?php echo $dados_ped['ds_coluna']; ?>&alt=<?php echo $dados_ped['ds_altura']; ?>&qtd=<?php echo $totalQtde; ?>&galpao=<?php echo $dados_ped['ds_galpao']; ?>&cod_estoque=<?php echo $dados_ped['cod_estoque']; ?>&cod_col=<?php echo $dados_ped['cod_col']; ?>" class="btn btn-primary btn-lg btn-block" data-transition="pop" style="text-decoration: none">
							...
							<fieldset>
								<div class="form-group">
									...
									<button type="submit" class="ui-btn ui-shadow-icon ui-btn-a" id="btnFinConfPedCegoSerial" value="<?php echo $nr_pedido;?>" style="float: right;margin-left: 5px;background-color: #16a085;text-shadow: none;color:white;border-color: #fdfbfb">FINALIZAR</button>
								</div>
							</fieldset>
						</form>
					</div>
