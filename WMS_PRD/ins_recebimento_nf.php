...
<legend>CADASTRAR NOTA FISCAL
  <button type="button" class="btn btn-success btn-xs" id="btnInsNfRec" value="<?php echo $id_rec;?>" style="float: right;width: 150px">SALVAR</button>
</legend>
        ...
        <div class="col-md-5">
          <div class="input-group input-group-md">
            ...
            <span class="input-group-btn">
              <button class="btn btn-info" type="button" id="btnConsEmitNf" style="height: 32px"><span class="fa fa-search"></span></button>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-md-5">
          <div class="input-group input-group-md">
            ...
            <span class="input-group-btn">
              <button class="btn btn-info" type="button" id="btnInsEmitNf" style="height: 32px"><span class="fa fa-plus" title data-original-title="Cadastrar Emitente"></span></button>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('#btnInsEmitNf').on('click',function(e){
      event.preventDefault();
      $('#retornoNf').load("data/entidade/modal/m_ins_cliente_nf.php");
    });

    $('#btnInsDestNf').on('click',function(e){
      event.preventDefault();
      $('#retornoNf').load("data/entidade/modal/m_ins_destinatario_nf.php");
    });

    $('#btnConsEmitNf').on('click',function(){
      event.preventDefault();
      var nfe_chv = $('#nfe_chave').val();
      $.ajax
      ({
        url:"data/recebimento/consulta_dados_chave.php",
        method:"POST",
        dataType:'json',
        data:{
          nfe_chv :nfe_chv
