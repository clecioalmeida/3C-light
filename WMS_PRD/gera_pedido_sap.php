...
group by t1.ds_destino order by t1.ds_destino";
$res = mysqli_query($link, $sql) or die(mysqli_error($link));
while ($dados_nf=mysqli_fetch_assoc($res)) {
	echo "ds_destino: ".$dados_nf['ds_destino']."-".$dados_nf['cod_prod_cliente']."<br>";
	...
	if(mysqli_affected_rows($link1) > 0){
		...
		$sql_prd = "select * 
		from tb_mb51e 
		where fl_empresa = '$cod_cli' and ds_destino = '".$dados_nf['ds_destino']."' and nr_qtde > 0 and nr_pedido is null group by cod_prod_cliente";
		$res_prd = mysqli_query($link, $sql_prd) or die(mysqli_error($link));
		while ($dados_pr=mysqli_fetch_assoc($res_prd)) {

			echo 'Pedido: '.$pedido_novo.' ds_destino: '.$dados_pr['ds_destino'].' cod_prod_cliente: '.$dados_pr['cod_prod_cliente'].' nr_qtde: '.$dados_pr['nr_qtde'].' cadastrado com sucesso.<br>';

			$ins_prd = "insert into tb_pedido_coleta_produto (
				nr_pedido, produto, fl_status, fl_empresa, nr_qtde, usr_create, dt_create
				) values (
					'".$pedido_novo."', '".$dados_pr['cod_prod_cliente']."',  'A', '".$cod_cli."', '".$dados_pr['nr_qtde']."', '".$id."', '".$date."')";
