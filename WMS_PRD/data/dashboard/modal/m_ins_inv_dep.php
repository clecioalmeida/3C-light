      ...
      <div class="modal-content">
        ...
        <div class="modal-body modal-lg" style="overflow-y: auto">
          ...
          <fieldset>
            ...
            <section>
              <div class="form-group">
                <label class="col-sm-2 control-label" for="nm_produto">ACUR VALOR</label>
                <div class="col-sm-2">
        ...
        <div class="modal-footer modal-lg" style="background-color: #22262E;">
          <button type="submit" class="btn btn-primary" id="btnSaveInvDep" value="<?php echo $upd_rec;?>">SALVAR</button>
  ...
  $(document).ready(function () {
    ...
    $(document).on('change','#nr_sku_falta', function(){
      ...
      if($('#nr_sku_qtde').val() == '' || $('#nr_sku_sobra').val() == '' || $('#nr_sku_falta').val() == ''){
        ...
        var qtd = parseFloat($('#nr_sku_qtde').val()) - parseFloat($('#nr_sku_sobra').val()) - parseFloat($('#nr_sku_falta').val());
    ...
    $('#vlr_ini').on('change', function(){
      ...
      vlr_ini = parseFloat($('#vlr_ini').val().replace('.','').replace(',','.'));
    ...
    $('#vlr_sobra').on('change',function(){
      ...
      vlr_sobra =  parseFloat($('#vlr_sobra').val().replace('.','').replace(',','.'));
    ...
    $('#vlr_falta').on('change', function(){
      ...
      vlr_falta = parseFloat($('#vlr_falta').val().replace('.','').replace(',','.'));
      ...
      if($('#vlr_ini').val() == '' || $('#vlr_sobra').val() == '' || $('#vlr_falta').val() == ''){
        ...
        console.log(vlr_ini+" teste");

        vlr = (vlr_ini - vlr_sobra) + vlr_falta;
        perc_vlr = (vlr / vlr_ini)*100;
        acur_vlr = perc_vlr.toFixed(2);
        vlr2 = vlr.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"});
        $('#vlr_fim').val(vlr2);
        $('#vlr_div').val(acur_vlr);
        console.log(perc_vlr);
        console.log(acur_vlr);
        console.log(vlr2);
