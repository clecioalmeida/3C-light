...
if($novo_pedido > 0){
	...
	if($msg > 0){
		...
		$ins_pedido = "insert into tb_pedido_coleta (nr_pedido, id_remetente, dt_imite, nm_usuario, dt_pedido, fl_status, fl_tipo) values ('$novo_pedido', 110, now(), 2, now(), 'A', 'A')";
		$res_pedido = mysqli_query($link, $ins_pedido) or die(mysqli_error($link1));

	}else{

		$retorno[] = array(

			'info'	=> "Não há novos arquivos.",
		);
