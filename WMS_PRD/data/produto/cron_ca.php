...
from tb_ca t1
left join tb_posicao_pallet t2 on t1.id = t2.cod_ca and coalesce(t2.fl_bloq,'N') = 'N'
...
where DATEDIFF(date(t1.dt_docto), now()) <= 30 and DATEDIFF(date(t1.dt_docto), now()) > 0 and t2.nr_qtde > 0";
...
while ($dados = mysqli_fetch_assoc($res)) {
	...
	$sql_fun = "select t5.cod_col, t5.nr_pedido, t6.cod_almox, upper(t7.ds_nome) as ds_nome, date_format(t6.dt_pedido,'%d/%m/%Y') as dt_pedido, t5.cod_estoque
	...
	while ($dados_func = mysqli_fetch_assoc($res_fun)) {
		...
		$html .= '<tr><td>Matrícula:</td><td>'.$dados_func['cod_almox'].'</td><td>Nome:</td><td>'.$dados_func['ds_nome'].'</td></tr>';
