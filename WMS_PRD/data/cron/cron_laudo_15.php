...
where t2.nr_qtde > 0 and DATEDIFF(date(t1.dt_docto), date(now())) = '15'";
...
if(mysqli_num_rows($res) > 0){
	...
	$html .= '<tr><td><p>1.Todo material que será bloqueado 15 dias antes do vencimento da validade.</p>';
	$html .= '</table></div>';
	...
	if ($enviado) {
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 15 dias', 'OK', '99', '$date')";
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 15 dias', 'ERRO', '99', '$date')";
		$res_alert = mysqli_query($link, $sql_alert);
	}
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 15 dias', 'VAZIO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
