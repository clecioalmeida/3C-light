...
$sql = "select t1.id, t1.nr_docto, t1.dt_docto, DATEDIFF(date(t1.dt_docto), date(now())) as qtd_dias, t2.produto, t3.nm_produto, t2.cod_estoque, t2.ds_galpao, t4.nome, t2.ds_prateleira, t2.ds_coluna, t2.ds_altura, round(t2.nr_qtde,0) as nr_qtde, coalesce(t2.fl_bloq,'N') as fl_bloq, t5.cod_col, t5.nr_pedido, upper(t7.ds_nome) as ds_nome, date_format(t6.dt_pedido,'%d/%m/%Y') as dt_pedido, t5.cod_estoque
from tb_ca t1
left join tb_posicao_pallet t2 on t1.id = t2.cod_laudo and coalesce(t2.fl_bloq,'N') = 'N'
...
where DATEDIFF(date(t1.dt_docto), now()) <= 30 and DATEDIFF(date(t1.dt_docto), now()) > 0";
...
<div id="logo-group">
<span id="logo" style="margin-top: 5px"> <img src="http://localhost/DSV/Site/data/recebimento_edp/img/logo12.png" alt="Argus" style="width:160px;height:60px"></span>
<span id="logo" style="margin-top: 3px"> <img src="http://localhost/DSV/Site/data/recebimento_edp/img/logo2.png" alt="3C" style="width:140px;height:50px"></span>
</div>
...
while ($dados = mysqli_fetch_assoc($res)) {
	...
	$sql_fun = "select t1.cod_col, t5.nr_pedido, t6.cod_almox, upper(t7.ds_nome) as ds_nome, date_format(t6.dt_pedido,'%d/%m/%Y') as dt_pedido, t5.cod_estoque, t7.cod_depto
...
if ($enviado) {
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 0 dias', 'OK', '99', '$date')";
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 0 dias', 'ERRO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 0 dias', 'VAZIO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
