...
where DATEDIFF(date(t1.dt_docto), now()) <= '5' and DATEDIFF(date(t1.dt_docto), now()) > '0'";
...
if(mysqli_num_rows($res) > 0){
	...
	if ($enviado) {
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 5 dias', 'OK', '99', '$date')";
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 5 dias', 'ERRO', '99', '$date')";
		$res_alert = mysqli_query($link, $sql_alert);
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'LAUDO 5 dias', 'VAZIO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
