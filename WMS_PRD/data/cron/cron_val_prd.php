...
where t1.nr_qtde > 0 and DATEDIFF(date(t1.dt_validade), date(now())) = '15'";
...
if(mysqli_num_rows($res) > 0){
	...
	$mail->AddAddress('eduardomenocio@growupsti.com.br');
	$mail->AddCC($email_principal, 'Operação logística'); 
	...
	if ($enviado) {
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'VAL 0 dias', 'OK', '99', '$date')";
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'VAL 0 dias', 'ERRO', '99', '$date')";
		$res_alert = mysqli_query($link, $sql_alert);
	}
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'VAL 0 dias', 'VAZIO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
