...
from tb_ca t1
left join tb_posicao_pallet t2 on t1.id = (CASE t1.fl_tipo WHEN 'C' then t2.cod_ca when 'L' THEN t2.cod_laudo END)
...
where DATEDIFF(date(t1.dt_docto), date(now())) = '15'";
...
if(mysqli_num_rows($res) > 0){
	...
	$html = '<header id="header">
				...
				<div style="float: center">
					<h3 style="color: #FF8C00"><strong>ALERTA DE BLOQUEIO PRODUTOS ALOCADOS</strong></h3>
				</div>
			</header>';
	$html .= '<div><table>';
	...
	while ($dados = mysqli_fetch_assoc($res)) {
		...
		$upd_prd = "update tb_posicao_pallet set fl_bloq = 'S' where cod_estoque = '".$dados['cod_estoque']."'";
		$res_upd = mysqli_query($link, $upd_prd);

		if(mysqli_affected_rows($link) > 0){

			$ins_log = "insert into tb_log_produto (cod_item, ds_ref, id_ref, ds_rotina, ds_obs, usr_create, dt_create) value ('".$dados['produto']."', 'COD_ESTOQUE', ".$dados['cod_estoque']."', 'CONTROLE DE CA', 'PRODUTO BLOQUEADO', '99', '$date')";
			$res_log = mysqli_query($link, $ins_log);

			$html .= '<tr><td>Situação:</td><td>'.$dados['produto'].' bloqueado.</td></tr>';

		}else{

			$ins_log = "insert into tb_log_produto (cod_item, ds_ref, id_ref, ds_rotina, ds_obs, usr_create, dt_create) value ('".$dados['produto']."', 'COD_ESTOQUE', ".$dados['cod_estoque']."', 'CONTROLE DE CA', 'ERRO NO BLOQUEIO DO PRODUTO', '99', '$date')";
			$res_log = mysqli_query($link, $ins_log);
	...
	if ($enviado) {
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'BLOQ CA 15 dias', 'OK', '99', '$date')";
		...
		$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'BLOQ CA 15 dias', 'ERRO', '99', '$date')";
		$res_alert = mysqli_query($link, $sql_alert);
	}
	...
	$sql_alert = "insert into tb_log_ca (dt_alerta, tp_alerta, fl_status, usr_create, dt_create) values ('$date', 'BLOQ CA 15 dias', 'VAZIO', '99', '$date')";
	$res_alert = mysqli_query($link, $sql_alert);
