...
if ($fl_status == "A") {
	...
	while ($dados_prd = mysqli_fetch_array($res_prd)) {
		...
		$sql_pp = "SELECT distinct p.cod_estoque, p.ds_galpao, p.ds_prateleira, p.ds_coluna, p.ds_altura, p.nr_qtde, g.nome, p.dt_validade,  c.dt_docto as dt_ca, l.dt_docto as dt_laudo, e.id as cod_etq
		...
		WHERE p.produto = '$produto' and p.nr_qtde > 0 and coalesce(p.fl_bloq,'N') = 'N' and p.fl_status <> 'E'
		order by p.dt_validade, c.dt_docto, l.dt_docto, p.nr_qtde, p.ds_galpao, p.ds_prateleira, p.ds_coluna, p.ds_altura";
		...
		if(mysqli_num_rows($res_pp) > 0){
			...
			while ($dados_qtd = mysqli_fetch_array($res_pp)) {
				...
				//echo "cod_estoque - ".$cod_estoque." produto - ".$produto." - "."Qtde pedido - ".$nr_qtde."-"."Qtde parcial - ".$qtde_parcial."-"."Produto - ".$produto."-"."Qtde posicao - ".$nr_qtde_pp."<br>";
