...
$sql_ped = "select t1.nr_pedido, date_format(t3.dt_limite,'%d/%m/%Y') as dt_limite, t1.produto, t2.nm_produto, t4.cod_almox, t4.ds_almox, t1.nr_qtde, coalesce(t1.nr_qtde_conf,0) as nr_qtde_conf
...
where t3.fl_tipo = 'P' and coalesce(t1.nr_qtde_conf,0) < t1.nr_qtde and date(t3.dt_limite) >= '$dt_ini' and date(t3.dt_limite) <= '$dt_fim' and t3.fl_empresa = '$cod_cli'
order by t1.nr_pedido";
			...
			<tbody>
				<?php while ($linha = mysqli_fetch_array($ped)) {
					...
					<tr class="status" data-status="<?php echo $linha['fl_status']; ?>">
						...
						<td> <?php echo $linha['produto']."-".$linha['nm_produto'];?></td>
						<td> <?php echo $linha['nr_qtde'];?></td>
						<td> <?php echo $linha['nr_qtde_conf'];?></td>
						<!--td class="status"> 
							<?php
