...
if ($fl_status == "A") {
	...
	while ($dados_prd = mysqli_fetch_array($res_prd)) {
		...
		WHERE p.produto = '$produto' and p.nr_qtde > 0  and (p.fl_bloq = 'N' or p.fl_bloq is null) and p.fl_empresa = '$cod_cli' and p.fl_status <> 'E'
		...
		if(mysqli_num_rows($res_pp) > 0){
			...
			$ins_prd = "insert into tb_coleta_pedido (nr_pedido,produto,nr_qtde_col,usr_create,dt_create,fl_status, cod_estoque) values('$nr_pedido', '$produto', '$nr_qtde', '$id', now(), 'M')";
			$res_ins = mysqli_query($link, $ins_prd);
			...
			$upd_col = "update tb_pedido_coleta_produto set usr_init_col = '$id', dt_init_col = now(), fl_status = 'R' where nr_pedido = '$nr_pedido'";
			$result_upd = mysqli_query($link1, $upd_col);
