...
while ($dest=mysqli_fetch_assoc($res_dest)) {
    ...
    $sql_prd = "update tb_pedido_coleta_produto set fl_status = 'W', nr_qtde_conf = '$qtde_conf', usr_fim_coleta = '$id', dt_fim_coleta = '$date' where produto = '$produto' and nr_pedido = '$nr_pedido'";
...
if(mysqli_affected_rows($link) > 0){ 
    ...
    $sql = "insert into tb_ocorrencias (nm_ocorrencia, tipo, ds_responsavel, nm_depto, criticidade, dt_abertura, fl_status, cod_origem, ds_obs, fl_empresa, user_create, dt_create) values ('200 - Pedido liberado parcialmente', '', 'id', 'Separação', 'A', '$date', 'A', '$nr_pedido', '', '$cod_cli', '$id', '$date')";
    $resultado_id = mysqli_query($link, $sql);
