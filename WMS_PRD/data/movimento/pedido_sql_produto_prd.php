...
$select_produto = "select t2.nr_pedido, t2.cod_ped, t2.produto, t2.nr_qtde as qtde,t2.nr_volume, t3.nm_produto, t3.cod_produto, t3.cod_prod_cliente, t4.fl_status, t4.doc_material, t5.ds_prateleira, t5.ds_coluna, t5.ds_altura, CONCAT(t6.nr_matricula,'-', upper(t6.ds_nome)) as nm_func, date_format(t2.dt_fim_conf,'%d/%m/%Y') as dt_fim_conf
...
left join tb_pedido_conferencia t5 on t2.nr_pedido = t5.nr_pedido and t2.produto = t5.produto
left join tb_funcionario t6 on t4.cod_almox = t6.nr_matricula and t6.fl_status <> 'E'
where t2.produto = '$cod_prd' and t2.fl_empresa = '$cod_cli'
group by t2.nr_pedido";
			...
			<tbody id="retPrdPedido">
				...
				while($dados_produto=mysqli_fetch_assoc($res_produto)){?>
					<tr class="odd gradeX">
						...
						<td class="atualiza"> <?php echo $dados_produto['dt_fim_conf']; ?> </td>
						<td class="atualiza"> <?php echo $dados_produto['nm_func']; ?> </td>
