...
where t1.user_create = '$id_usr' and t1.fl_status = 'A' and t1.fl_empresa = '$cod_cli'
GROUP BY t1.id";
...
while ($dados_conf = mysqli_fetch_assoc($res_conf)) {
  ...
  if ($cod_estoque > 0) {
    ...
    $ins_pos = "INSERT into tb_posicao_pallet (
      produto, ds_galpao, ds_prateleira, ds_coluna, ds_altura, nr_volume, nr_qtde, ds_lp, id_endereco, n_serie, ds_kva, ds_ano, 
        ds_fabr, fl_status, fl_tipo, fl_bloq, fl_empresa, id_tar, usr_create, dt_create
      ) values (
        '$cod_produto', '$id_galpao', '$id_rua', '$id_coluna', '$id_altura', '$nr_volume', '$qtd_inv', 
        '$ds_lp', '$id_end', '$ds_serial', '$ds_kva', '$ds_ano', '$ds_fabr', 'A', 'I', 'N', '$cod_cli', '$id_tar', '$id_user', '$date'
        )";
...
from tb_inv_tarefa
where user_create = '$id_usr' and fl_status = 'A' and fl_empresa = '$cod_cli'";
