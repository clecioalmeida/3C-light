...
$sql_local = "SELECT t1.id, t1.nome, t2.cod_estoque, t2.ds_galpao, t2.produto, t2.ds_galpao, t2.ds_prateleira, t2.ds_coluna, t2.ds_altura, round(t2.nr_qtde,0) as nr_qtde, case when t2.dt_validade = '0' then '' else date_format(t2.dt_validade,'%d/%m/%Y') end as dt_validade, t6.nr_docto as cod_ca, date_format(t6.dt_docto,'%d/%m/%Y') as dt_ca, t7.nr_docto as cod_laudo, date_format(t7.dt_docto,'%d/%m/%Y') as dt_laudo, t3.cod_prod_cliente, t3.nm_produto, t2.nr_or, date_format(t4.dt_recebimento_real,'%d/%m/%Y') as dt_recebimento, t2.id_tar, date_format(t5.dt_create,'%d/%m/%Y') as dt_tarefa, t2.n_serie, t4.ds_lp, t4.ds_ano, t4.ds_fabr
from tb_armazem t1
left join tb_posicao_pallet t2 on t1.id = t2.ds_galpao
left join tb_produto t3 on t2.produto = t3.cod_prod_cliente
left join tb_recebimento_ag t4 on t2.nr_or = t4.cod_recebimento
...
where t2.fl_empresa = '$cod_cli' and t2.nr_qtde > 0 and t2.fl_status = 'A' and t2.produto > 0
                ...
                <tbody>
                    ...
                    ?>
                        <tr class="odd gradeX">
                            ...
                            <td style="text-align: center; width: 5px"> <?php echo $dados_local['n_serie']; ?></td>
                            <td style="text-align: center; width: 5px"> <?php echo $dados_local['ds_lp']; ?></td>
                            <td style="text-align: center; width: 5px"> <?php echo $dados_local['ds_ano']; ?></td>
                            <td style="text-align: center; width: 5px"> <?php echo $dados_local['ds_fabr']; ?></td>
                            ...
                            <td style="text-align: right; width: auto"> <?php echo $dados_local['dt_tarefa']; ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
