...
$sql_local = "select t1.id, t1.nome, t2.cod_estoque, t2.ds_galpao, t2.produto, t2.ds_galpao, t2.ds_prateleira, t2.ds_coluna, t2.ds_altura, format(t2.nr_qtde,0) as nr_qtde, case when t2.dt_validade = '0' then '' else date_format(t2.dt_validade,'%d/%m/%Y') end as dt_validade, t6.nr_docto as cod_ca, date_format(t6.dt_docto,'%d/%m/%Y') as dt_ca, t7.nr_docto as cod_laudo, date_format(t7.dt_docto,'%d/%m/%Y') as dt_laudo, t3.cod_prod_cliente, t3.nm_produto, t2.nr_or, date_format(t4.dt_recebimento_real,'%d/%m/%Y') as dt_recebimento, t2.id_tar, date_format(t5.dt_create,'%d/%m/%Y') as dt_tarefa
from tb_armazem t1
...
where t2.fl_empresa = '$cod_cli' and t2.nr_qtde > 0 and t2.fl_status = 'A' and t2.produto > 0 and fl_bloq = 'S'
...
if ($tr_local) { ?>
    ...
    <div id="reportSalEstoque">
        <div class="padding-10">
            ...
            <br>
            <br>
            <table class="display responsive nowrap" id="example13" style="width:100%">
                <thead>
                    <tr>
                        <th> Código </th>
                        <th> Rua </th>
