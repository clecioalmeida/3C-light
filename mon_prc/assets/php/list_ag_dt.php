...
$dt_ini_ag  = trim(strip_tags($_REQUEST['dt_ini_ag']));
$dt_fim_ag  = trim(strip_tags($_REQUEST['dt_fim_ag']));
...
$sql .= "WHERE (r.dt_recebimento_real >= ? and r.dt_recebimento_real <= ?) ";
$sql .= "GROUP BY r.cod_recebimento ORDER BY r.dt_recebimento_real desc";
$stm = $conexao->prepare($sql);
$stm->bindValue(1, $dt_ini_ag);
$stm->bindValue(2, $dt_fim_ag);
...
if (!empty($retorno)) {
	...
	$retorno[] = array('info' => '1', 'count' => $count);
	echo json_encode($retorno);
}